...
func (repo *userRepository) Find(
	...
	buy_amount_p4, buy_slippage, sell_amount_p1, sell_amount_p2, sell_amount_p3, sell_slippage, sell_protection
...
func (repo *userRepository) Update(
	...
	query := fmt.Sprintf(`
		...
		RETURNING telegram_id, bot_language, accept_agreement, wallet_address,
		          ...
		          sell_slippage, sell_protection`, strings.Join(setClauses, ", "), argIndex)
...
func scan(row *sql.Row) (*model.User, error) {
	...
	if err := row.Scan(
		...
		&user.BuyAmountP4, &user.BuySlippage, &user.SellAmountP1,
		&user.SellAmountP2, &user.SellAmountP3,
